...
const Header: React.FC = () => {
  ...
  const isActive = (path: string) => {
    if (path === "/services") {
      return (
        location.pathname === path || location.pathname.includes("/services")
      );
    }
    if (path === "/team") {
  ...
  return (
    <AppBar
      position="sticky"
      sx={{ backgroundColor: "white", color: "primary.main", boxShadow: 2 }}
    >
        ...
        <Toolbar>
          ...
          >
                      ...
                      >
                        {serviceItems.map((item, index) => (
                          <MenuItem
                            ...
                            to={item.path}
                            onClick={() =>
                              handleServiceItemClick(item.tabIndex)
                            }
                            sx={{
                              py: 1,
                              ...(index === 0 && {
            ...
            <Button
              ...
              sx={{
                color: isActive("/app/cycle-tracking")
                  ? "primary.main"
                  : "text.secondary",
                fontWeight: isActive("/app/cycle-tracking") ? "bold" : "normal",
                borderBottom: isActive("/app/cycle-tracking")
                  ? "2px solid"
                  : "none",
                borderRadius: 0,
                "&:hover": {
